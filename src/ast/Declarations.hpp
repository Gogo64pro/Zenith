...
export namespace zenith {
	struct VarDeclNode : StmtNode {
		...
		[[nodiscard]] std::string toString(int indent = 0) const override {
			...
			return fmt::format("{pad}{orange}{isConst} {isHoisted} {light_purple}{kind} {yellow}{name}{init}{reset}",
			                   ...
			                   fmt::arg("name", name),
			                   fmt::arg("init", initializer
				                                    ? fmt::format(" {white}= {init}",
				                                                  fmt::arg("init", removePadUntilNewLine(
	...
	struct FunctionDeclNode : virtual ASTNode, virtual IAnnotatable {
		struct Param {
			std::string name;
			polymorphic<TypeNode> type;
			polymorphic<ExprNode> defaultValue;

			Param(std::string name, polymorphic<TypeNode> type = nullptr,
		...
		FunctionDeclNode(
			...
			polymorphic<BlockNode> body,
			bool isAsync = false,
			bool structSugar = false,
			std::vector<polymorphic<AnnotationNode> > ann = {}) : ASTNode(),
			                                                      name(std::move(name)), params(std::move(params)),
			                                                      returnType(std::move(returnType)),
			                                                      body(std::move(body)),
			                                                      isAsync(isAsync), usingStructSugar(structSugar) {
			this->loc = std::move(loc);
		...
		[[nodiscard]] std::string toString(int indent = 0) const override {
			...
			for (const auto &param: params) {
				...
				if (param.type) {
					param_str += param.type->toString() + " ";
				}

				param_str += fmt::format("{}{}{}", CL_WHITE, param.name, RESET_COLOR);

				if (param.defaultValue) {
					param_str += fmt::format("{} = {}{}", CL_WHITE,
					                         param.defaultValue->toString(indent + 2),
					                         RESET_COLOR);
				}

				formatted_params.push_back(std::move(param_str));
			}

			std::string args_str = formatted_params.empty()
				                       ? ""
				                       : fmt::format("{}", fmt::join(formatted_params,
			...
			return fmt::format(
				...
				fmt::arg("name", fmt::format("{}{}{}", CL_YELLOW, isLambda() ? "[LAMBDA]" : name, RESET_COLOR)),
				fmt::arg("white", CL_WHITE),
				fmt::arg("paren_open", usingStructSugar ? "({" : "("),
				fmt::arg("paren_close", usingStructSugar ? "}) " : ") "),
				fmt::arg("args", args_str),
				fmt::arg("body", body ? removePadUntilNewLine(body->toString(indent + 2)) : ""),
				fmt::arg("reset", RESET_COLOR)
			);
		}
	...
	// Base MemberDeclNode
	struct MemberDeclNode : virtual ASTNode, virtual IAnnotatable {
		enum class Kind : uint8_t { FIELD, METHOD, METHOD_CONSTRUCTOR, MESSAGE_HANDLER };
		enum class Access : uint8_t { PUBLIC, PROTECTED, PRIVATE, PRIVATEW, PROTECTEDW };
...
#pragma pack(pop)
	...
	protected:
		...
		[[nodiscard]] std::string buildMemberHeader(int indent) const {
			...
			// Output access, const, static, kind, and name
			ss << pad << accessNames[static_cast<int>(flags.access)]
			   << (flags.isConst ? " CONST" : "")
			   << (flags.isStatic ? " STATIC " : " ")
			   << kindNames[static_cast<int>(flags.kind)] << " " << name;
	...
	// FieldDeclNode
	struct FieldDeclNode : MemberDeclNode {
		polymorphic<TypeNode> type;
		polymorphic<ExprNode> initializer;
	...
	// Base MethodDeclNode
	struct MethodDeclNode : FunctionDeclNode, MemberDeclNode {
	    MethodDeclNode(
	        SourceLocation loc,
	        Access access,
	        bool isConst,
	        ...
	        )
	    {
	        this->loc = std::move(loc);
	    	this->annotations = std::move(ann);
	        MemberDeclNode::name = FunctionDeclNode::name;
	    }

	    [[nodiscard]] std::string toString(int indent = 0) const override {
	    	std::stringstream ss;
	    	ss << buildMemberHeader(indent) << FunctionDeclNode::toString(indent);
	...
	// CtorDeclNode
	struct CtorDeclNode : MethodDeclNode {
	    ...
	    {
	    	this->flags.kind = Kind::METHOD_CONSTRUCTOR;
	    	// Add initializers
	    ...
	    [[nodiscard]] std::string toString(int indent = 0) const override {
	    	...
	    	for (size_t i = 0; i < params.size(); ++i) {
	    		if (i > 0) ss << ", ";
	    		ss << params[i].name;
	    		if (params[i].type) ss << " : " << params[i].type->toString();
	    		if (params[i].defaultValue) ss << " = " << params[i].defaultValue->toString();
	    	}
	    	ss << ")";
	...
	// MessageHandlerNode
	struct MessageHandlerNode : MethodDeclNode {
		//todo actually add method handlers
		MessageHandlerNode(
			SourceLocation loc,
			Access access,
			bool isConst,
			...
			this->flags.kind = Kind::MESSAGE_HANDLER;
		}

		// Use MethodDeclNode's toString method
		[[nodiscard]] std::string toString(int indent = 0) const override {
	...
	struct ObjectDeclNode : ASTNode {
		...
		ObjectDeclNode(SourceLocation loc, Kind kind, std::string name, std::string base,
			...
			: name(std::move(name)), base(std::move(base)),
			  members(std::move(memb)), operators(std::move(ops)),
		...
		[[nodiscard]] std::string toString(int indent = 0) const override {
			std::string pad(indent, ' ');
			std::stringstream ss;
			ss << pad << (kind == Kind::CLASS ? "CLASS " : kind == Kind::STRUCT ? "STRUCT " : "ACTOR ")
					<< name;
			if (!base.empty())
				ss << " : " << base;
			ss << " {\n";
	...
	struct ActorDeclNode : ObjectDeclNode {
		...
		) : ObjectDeclNode(std::move(loc), ObjectDeclNode::Kind::ACTOR, std::move(name), std::move(baseActor),
		                   std::move(members)) {
		}
	...
	struct MultiVarDeclNode : StmtNode {
		...
		explicit MultiVarDeclNode(SourceLocation loc, std::vector<polymorphic<VarDeclNode> > &&vars) : vars(
			std::move(vars)) {
			this->loc = std::move(loc);
		}
